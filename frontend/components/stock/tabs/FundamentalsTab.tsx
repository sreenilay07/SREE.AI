    ...
    <div className="space-y-8">
      ...
      <section className="bg-gray-800 p-6 rounded-lg shadow-xl border border-gray-700">
        ...
        <div className="grid md:grid-cols-2 gap-x-8 gap-y-2">
          <RatioDisplay label="P/E Ratio" value={(data.keyRatios.pe || 0).toFixed(2)} term="P/E Ratio" lowerIsBetter />
          <RatioDisplay label="P/B Ratio" value={(data.keyRatios.pb || 0).toFixed(2)} term="P/B Ratio" lowerIsBetter />
          <RatioDisplay label="Debt-to-Equity" value={(data.keyRatios.debtToEquity || 0).toFixed(2)} term="Debt-to-Equity Ratio" lowerIsBetter />
          <RatioDisplay label="Return on Equity (RoE)" value={(data.keyRatios.roe || 0).toFixed(2)} unit="%" term="Return on Equity (RoE)" higherIsBetter />
          <RatioDisplay label="Dividend Yield" value={(data.keyRatios.dividendYield || 0).toFixed(2)} unit="%" term="Dividend Yield" higherIsBetter />
        </div>
      </section>
      ...
      <section className="bg-gray-800 p-6 rounded-lg shadow-xl border border-gray-700">
        ...
        <div className="grid md:grid-cols-1 lg:grid-cols-2 gap-6 items-center">
          ...
          <div className="text-sm">
            <p className="text-gray-300 mb-1"><span className="font-semibold">Promoter:</span> {(data?.shareholdingPattern?.promoter || 0).toFixed(2)}%</p>
            <p className="text-gray-300 mb-1"><span className="font-semibold">FII:</span> {(data?.shareholdingPattern?.fii || 0).toFixed(2)}%</p>
            <p className="text-gray-300 mb-1"><span className="font-semibold">DII:</span> {(data?.shareholdingPattern?.dii || 0).toFixed(2)}%</p>
            <p className="text-gray-300 mb-1"><span className="font-semibold">Public & Others:</span> {(data?.shareholdingPattern?.public || 0).toFixed(2)}%</p>
            {data?.shareholdingPattern?.pledgedPromoter !== undefined && (
              <p className={`mt-3 p-2 rounded ${promoterPledgeHigh ? 'bg-red-500/30 text-red-300' : 'bg-green-500/20 text-green-300'}`}>
                <span className="font-semibold">Promoter Pledged:</span> {(data?.shareholdingPattern?.pledgedPromoter || 0).toFixed(2)}%
