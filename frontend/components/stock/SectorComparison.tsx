          ...
          <tbody className="divide-y divide-gray-700">
            {peers.map(stock => (
              ...
              >
                ...
                <td className={`py-3 px-3 text-right font-mono font-semibold ${getChangeColor(stock.changePercent || 0)}`}>
                  {(stock.changePercent || 0).toFixed(2)}%
                </td>
                <td className="py-3 px-3 text-right font-mono text-gray-200">{stock.marketCap}</td>
                <td className="py-3 px-3 text-right font-mono text-gray-200">{stock.peRatio !== undefined ? stock.peRatio.toFixed(2) : 'N/A'}</td>
              </tr>
            ))}
          </tbody>
        </table>
