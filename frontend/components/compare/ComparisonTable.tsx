...
const ComparisonTable: React.FC<ComparisonTableProps> = ({ stocksData, isLoading }) => {
  ...
  const metrics = [
    { key: 'currentPrice', label: 'Last Price', format: (val: number) => `₹${(val || 0).toFixed(2)}` },
    { key: 'changePercent', label: '% Change', format: (val: number) => `${(val || 0).toFixed(2)}%`, higherIsBetter: true },
    { key: 'marketCap', label: 'Market Cap', format: (val: string) => val || 'N/A' },
    { key: 'peRatio', label: 'P/E Ratio', format: (val?: number) => val !== undefined ? val.toFixed(2) : 'N/A', lowerIsBetter: true },
    { key: 'pbRatio', label: 'P/B Ratio', format: (val?: number) => val !== undefined ? val.toFixed(2) : 'N/A', lowerIsBetter: true },
    { key: 'roe', label: 'RoE (%)', format: (val?: number) => val !== undefined ? val.toFixed(2) : 'N/A', higherIsBetter: true },
    { key: 'debtToEquity', label: 'Debt to Equity', format: (val?: number) => val !== undefined ? val.toFixed(2) : 'N/A', lowerIsBetter: true },
    { key: 'dividendYield', label: 'Dividend Yield (%)', format: (val?: number) => val !== undefined ? val.toFixed(2) : 'N/A', higherIsBetter: true },
    { key: 'promoterHolding', label: 'Promoter Holding (%)', format: (val?: number) => val !== undefined ? val.toFixed(2) : 'N/A', higherIsBetter: true },
    { key: 'volume', label: 'Per Day Volume', format: (val: number) => (val || 0).toLocaleString(), higherIsBetter: true },
  ];
